...
async fn metrics(
    ...
    Ok(format!(
        "# HELP zylith_renewal_relay_packages Registered renewal packages.\n\
         ...
         # HELP zylith_renewal_relay_awaiting_wallet_refresh_slots Slots blocked because reused funding notes already settled.\n\
...
fn extend_ops_alerts_for_package(
    ...
    if package.awaiting_wallet_refresh_slots > 0 {
        alerts.push(RelayOpsAlert {
            ...
            package_id: package_id.clone(),
            detail: "A prior child reused funding notes; refresh the package from the wallet before continuing.".into(),
        });
    }
    if let Some(observed_epoch) = last_observed_epoch {
            ...
            && package.end_epoch <= observed_epoch.saturating_add(expiry_warning_epochs)
        {
            alerts.push(RelayOpsAlert {
                ...
                package_id,
                detail: format!(
                    "Package ends at epoch {}; refresh before expiry to avoid missed renewal slots.",
                    package.end_epoch
                ),
            });
        }
        ...
        match fetch_proof_job_status(state, package, &prior.batch_id).await {
            Ok(Some(status)) => {
                if proof_job_failed(&status) {
                    return Some(slot_result(
                        ...
                        RelaySlotStatus::AwaitingSettlement,
                        Some(format!(
                            "Prior child batch {} proof failed; waiting for wallet refresh before reusing funding notes",
                            prior.batch_id
                        )),
                    ));
                }
                if proof_job_confirmed(&status) {
                    ...
                    if status.reuse_state.as_deref() == Some("matched") {
                        return Some(slot_result(
                            ...
                            RelaySlotStatus::AwaitingWalletRefresh,
                            Some(format!(
                                "Prior child batch {} settled with matched orders; refresh the package from the wallet before reusing funding notes",
                                prior.batch_id
                            )),
                        ));
                    }
                    return Some(slot_result(
                        ...
                        RelaySlotStatus::AwaitingSettlement,
                        Some(format!(
                            "Prior child batch {} is confirmed but no no-fill reuse attestation is available; waiting before reusing funding notes",
                            prior.batch_id
                        )),
                    ));
                }
                return Some(slot_result(
                    ...
                    RelaySlotStatus::AwaitingSettlement,
                    Some(format!(
                        "Prior child batch {} is not confirmed no-fill yet; waiting before reusing funding notes",
                        prior.batch_id
                    )),
                ));
            }
            Ok(None) | Err(_) => {
                return Some(slot_result(
                    ...
                    RelaySlotStatus::AwaitingSettlement,
                    Some(format!(
                        "Prior child batch {} status is unavailable; waiting before reusing funding notes",
                        prior.batch_id
                    )),
                ));
            }
