...
use zylith_core::{
    ...
    MultiPairArtifactBundleList, MultiPairArtifactBundleSummary, OutputCiphertextBundle, PairId,
    PublicSettlementTranscript, PublishedBatchArtifactList, PublishedBatchArtifactSummary,
    PublishedBatchArtifacts, PublishedMultiPairBatchArtifacts, SettlementRootHistoryArchive,
    SettlementRootHistoryBatch, SettlementTimestampUpdate, SettlementTranscript,
    WithdrawalAmountBucketPolicy, artifact_bundle_padded_count, artifact_epoch_bucket_end,
    artifact_epoch_bucket_start, count_bucket_label, extract_bearer_token,
...
async fn list_internal_root_history_by_epoch_range(
    ...
    batches.extend(
        multi_pair_artifacts
            ...
            .filter_map(|published| match multi_pair_root_history_batch(published) {
                Ok(batch) => Some(batch),
                Err(status) => {
                    eprintln!(
                        "skipping invalid multi-pair root-history artifact group_id={} status={status}",
                        published.transcript.group_id.0
                    );
                    None
                }
            }),
...
mod tests {
    ...
    fn sample_published_multi_pair_artifact(
        group_id: &str,
        batch_epoch: u64,
    ) -> PublishedMultiPairBatchArtifacts {
        let output_bundle =
            OutputCiphertextBundle::from_ciphertexts(BatchId(group_id.into()), "da-ref", vec![])
                .expect("multi-pair output bundle");
        let transcript = MultiPairSettlementTranscript {
            ...
            batch_bindings: vec![
                ...
                zylith_core::MultiPairSettlementBatchBinding {
                    batch_id: BatchId("batch-eth-usdc-11".into()),
            ...
            output_recovery_dummy_commitments: output_bundle
                .ciphertexts
                .iter()
    ...
    async fn archived_transcript_is_root_only_public_and_root_history_is_internal() {
        ...
        let mut multi_pair_artifacts = BTreeMap::new();
        multi_pair_artifacts.insert(
            "group-strk-eth-11".into(),
            sample_published_multi_pair_artifact("group-strk-eth-11", 11),
        );
        ...
        assert_eq!(history_json["batches"].as_array().unwrap().len(), 2);
        ...
        assert_eq!(history_json["batches"][1]["batch_id"], "group-strk-eth-11");
        assert_eq!(history_json["batches"][1]["pair_id"], "MULTI_PAIR");
        assert_eq!(
            history_json["batches"][1]["output_notes"]
                .as_array()
                .unwrap()
                .len(),
            0
