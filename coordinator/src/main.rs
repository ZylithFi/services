...
const HOSTED_RENEWAL_SLOT_LOOKAHEAD_EPOCHS: usize = 12;
const OPEN_ACCEPTING_BATCHES_PER_PAIR: usize = HOSTED_RENEWAL_SLOT_LOOKAHEAD_EPOCHS + 2;
...
struct AppState {
    ...
    is_multi_pair_group_settled_selector: String,
    verified_multi_pair_settlement_transcript_selector: String,
...
fn build_app_with_config(
    ...
    let app_state = AppState {
        ...
        is_multi_pair_group_settled_selector: selector_hex("is_multi_pair_group_settled"),
        verified_multi_pair_settlement_transcript_selector: selector_hex(
    ...
    Ok(Router::new()
        ...
        .route(
            ...
            post(publish_multi_pair_batch_artifacts),
        )
        .route(
...
fn public_settlement_transcript(
    ...
    Ok(PublicSettlementTranscript {
        ...
        multi_pair_commitment: published.transcript.multi_pair_commitment.clone(),
...
fn multi_pair_published_artifact_fingerprint(
    published: &PublishedMultiPairBatchArtifacts,
) -> Result<String, serde_json::Error> {
    let mut value = serde_json::to_value(published)?;
    if let Some(object) = value.as_object_mut() {
        object.remove("published_at_unix_ms");
...
fn multi_pair_settlement_timestamp_update_from_published(
    published: &PublishedMultiPairBatchArtifacts,
) -> Result<SettlementTimestampUpdate, StatusCode> {
    let roots = multi_pair_root_only_settlement_commitments(&published.transcript)
        .map_err(|_| StatusCode::BAD_REQUEST)?;
    let transcript_commitment = multi_pair_settlement_transcript_commitment(&published.transcript)
        .map_err(|_| StatusCode::BAD_REQUEST)?;
    let transaction_hash = published
        .settlement_transaction_hash
        .as_deref()
...
async fn verify_published_multi_pair_batch_artifact(
    state: &AppState,
    published: &PublishedMultiPairBatchArtifacts,
) -> Result<u64, StatusCode> {
    let update = multi_pair_settlement_timestamp_update_from_published(published)?;
    if state.require_artifact_onchain_verification {
        verify_multi_pair_settlement_timestamp_update(state, published, &update).await
    } else {
        published.settled_at_unix_ms.ok_or(StatusCode::BAD_REQUEST)
    }
}
...
async fn record_renewal_cancel_marker(
    ...
    persist_published_artifact_snapshots_if_configured(
        &state,
        &artifacts,
        &multi_pair_artifacts,
        &candidate,
    )?;
    *markers = candidate;
    Ok(Json(record))
...
async fn publish_multi_pair_batch_artifacts(
    State(state): State<AppState>,
    headers: HeaderMap,
    Path(group_id): Path<String>,
    Json(mut request): Json<PublishedMultiPairBatchArtifacts>,
) -> Result<Json<PublishedMultiPairBatchArtifacts>, StatusCode> {
    require_internal_auth(&state, &headers)?;
    ...
    persist_published_artifact_snapshots_if_configured(&state, &artifacts, &candidate, &markers)?;
...
async fn verify_multi_pair_settlement_timestamp_update(
    state: &AppState,
    published: &PublishedMultiPairBatchArtifacts,
    request: &SettlementTimestampUpdate,
) -> Result<u64, StatusCode> {
    ...
    let provided = normalize_required_felt(transcript_commitment)?;
    let expected_transcript = normalize_required_felt(
        &multi_pair_settlement_transcript_commitment(&published.transcript)
            .map_err(|_| StatusCode::BAD_REQUEST)?,
    )?;
    if provided != expected_transcript {
        return Err(StatusCode::BAD_REQUEST);
    }
    if !state.require_artifact_onchain_verification {
        return Ok(request.settled_at_unix_ms);
    ...
    let transaction_hash = request
        ...
        .ok_or(StatusCode::BAD_REQUEST)?;
    let block_ref = fetch_transaction_block_ref(state, transaction_hash).await?;
    let group_id = &published.transcript.group_id.0;
    let chain_settled = fetch_onchain_multi_pair_group_settled(
        state,
        &contract_address,
        group_id,
        block_ref.block_id.clone(),
    ...
    let chain_transcript = fetch_onchain_verified_multi_pair_settlement_transcript(
        state,
        &contract_address,
        group_id,
        block_ref.block_id,
...
async fn fetch_onchain_multi_pair_group_settled(
    state: &AppState,
    contract_address: &str,
    group_id: &str,
    block_id: serde_json::Value,
    ...
    let group_id_felt = encode_starknet_felt("batch-id", group_id);
    let settled = starknet_call_contract_at_block(
        state,
        &contract_address,
        &state.is_multi_pair_group_settled_selector,
        &[group_id_felt],
        block_id,
    )
    .await?
    ...
    .next()
    .ok_or(StatusCode::BAD_GATEWAY)?;
    Ok(normalize_required_felt(&settled)? != "0x0")
}

async fn fetch_onchain_verified_multi_pair_settlement_transcript(
    state: &AppState,
    contract_address: &str,
    group_id: &str,
    block_id: serde_json::Value,
    ...
    let transcript = starknet_call_contract_at_block(
        state,
        &contract_address,
        &state.verified_multi_pair_settlement_transcript_selector,
        &[group_id_felt],
        block_id,
    )
    .await?
...
async fn private_settlement_report_for_batch(
    ...
    let artifacts = state.published_batch_artifacts.read().await;
    if let Some(published) = artifacts.get(&batch_id) {
        return private_settlement_report_for_single_published(
            published,
            &requested_key_tags,
            &requested_order_auths,
        )
    ...
    let multi_pair_artifacts = state.published_multi_pair_batch_artifacts.read().await;
    let Some((settlement_id, published, member_binding)) =
        multi_pair_artifacts
            .iter()
            .find_map(|(group_id, published)| {
                if group_id == &batch_id {
                    return Some((group_id, published, None));
                }
                published
                    .transcript
                    .batch_bindings
                    .iter()
                    .find(|binding| binding.batch_id.0 == batch_id)
                    .map(|binding| (group_id, published, Some(binding)))
            })
    else {
        return Err(StatusCode::NOT_FOUND);
    };
    private_settlement_report_for_multi_pair_published(
        &batch_id,
        settlement_id,
        published,
        member_binding,
...
fn private_settlement_report_for_single_published(
    published: &PublishedBatchArtifacts,
    requested_key_tags: &BTreeSet<String>,
    requested_order_auths: &BTreeMap<String, BTreeSet<String>>,
) -> Result<PrivateSettlementReport, StatusCode> {
    let settled_at_unix_ms = published.settled_at_unix_ms.ok_or(StatusCode::NOT_FOUND)?;
    let order_execution_reports =
        matching_order_execution_reports(&published.order_execution_reports, requested_order_auths);
    let output_recovery_records = matching_output_recovery_records(
        &published.transcript.output_notes,
        &published.transcript.output_recovery_records,
        requested_key_tags,
...
fn private_settlement_report_for_multi_pair_published(
    requested_batch_id: &str,
    settlement_id: &str,
    published: &PublishedMultiPairBatchArtifacts,
    member_binding: Option<&zylith_core::MultiPairSettlementBatchBinding>,
    requested_key_tags: &BTreeSet<String>,
    requested_order_auths: &BTreeMap<String, BTreeSet<String>>,
) -> Result<PrivateSettlementReport, StatusCode> {
    let settled_at_unix_ms = published.settled_at_unix_ms.ok_or(StatusCode::NOT_FOUND)?;
    ...
    let roots = multi_pair_root_only_settlement_commitments(&published.transcript)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    let pair_id = member_binding
        .map(|binding| binding.pair_id.clone())
        .unwrap_or_else(|| PairId("MULTI_PAIR".into()));
    let price_base_scale = member_binding
        .map(|binding| binding.price_base_scale)
        .unwrap_or(1);

    Ok(PrivateSettlementReport {
        batch_id: BatchId(requested_batch_id.to_owned()),
        settlement_id: Some(BatchId(settlement_id.to_owned())),
        pair_id,
        batch_epoch: published.transcript.batch_epoch,
        settled_at_unix_ms,
        output_note_root: roots.output_note_root,
...
fn matching_order_execution_reports(
    reports: &[zylith_core::OrderExecutionReport],
    requested_order_auths: &BTreeMap<String, BTreeSet<String>>,
) -> Vec<zylith_core::OrderExecutionReport> {
    if requested_order_auths.is_empty() {
        return Vec::new();
    }
    reports
        .iter()
        .filter(|report| {
            ...
            let Ok(commitment) = zylith_core::hash::normalize_felt_hex(&report.order_commitment.0)
            else {
                return false;
            };
            requested_order_auths
...
fn matching_output_recovery_records(
    output_notes: &[zylith_core::OutputNoteRecord],
    recovery_records: &[zylith_core::OutputRecoveryRecord],
    requested_key_tags: &BTreeSet<String>,
    order_execution_reports: &[zylith_core::OrderExecutionReport],
) -> Vec<PrivateSettlementOutputRecoveryRecord> {
    let authenticated_output_commitments = order_execution_reports
        ...
        .collect::<BTreeSet<_>>();
    let authenticated_output_indices = output_notes
        .iter()
        .enumerate()
...
fn internal_proof_work_batch_summaries_limited<'a>(
    ...
    let mut summaries = records
        .filter(|record| !record.orders.is_empty())
...
fn load_published_batch_artifacts_store_file(path: &FsPath) -> PublishedBatchArtifactsStoreFile {
    let store = if is_sqlite_store(path) {
        PublishedBatchArtifactsStoreFile {
            ...
            multi_pair_artifacts_by_group: load_sqlite_records(
                path,
                "published_multi_pair_batch_artifacts",
            )
            .unwrap_or_default(),
...
fn persist_published_artifact_snapshots_if_configured(
    ...
    persist_published_batch_artifacts_store(path, artifacts, multi_pair_artifacts, markers)
}
...
mod tests {
    ...
    use zylith_core::{
        BatchId, BatchStatus, EncryptedRecoveryPayload, OrderCancellationRequest,
        OrderIngressClientTelemetry, OrderShareBundle, OrderSubmission, PairId, ProductConfig,
        PublishedBatchArtifacts, RecoveryArtifact, RecoveryArtifactKind, RecoveryArtifactUpload,
    ...
    #[test]
    fn multi_pair_private_report_returns_member_batch_with_group_settlement_id() {
        let group_id = "multi-pair-42";
        let member_batch_id = "batch-strk-usdc-42";
        let member_binding = zylith_core::MultiPairSettlementBatchBinding {
            batch_id: BatchId(member_batch_id.into()),
            pair_id: PairId("STRK/USDC".into()),
            batch_epoch: 42,
        ...
        let order_commitment = zylith_core::OrderCommitment("0xabc".into());
        let output_note = zylith_core::OutputNoteRecord {
        ...
        let published = zylith_core::PublishedMultiPairBatchArtifacts {
            transcript: transcript.clone(),
            output_bundle: zylith_core::OutputCiphertextBundle {
                batch_id: BatchId(group_id.into()),
                bundle_commitment: "0x555".into(),
                ciphertext_envelope_commitment: Some("0x666".into()),
            ...
            settlement_transaction_hash: Some("0xaaa".into()),
            settlement_contract_address: Some("0xbbb".into()),
            order_execution_reports: vec![
                zylith_core::OrderExecutionReport {
                    batch_id: BatchId(member_batch_id.into()),
                    pair_id: PairId("STRK/USDC".into()),
                    order_commitment: order_commitment.clone(),
                    order_report_auth_tag: Some("tag-1".into()),
                    ...
                    execution_preference: zylith_core::ExecutionPreference::PrivateThenExternal,
                    submitted_amount: 10,
                    filled_amount: 10,
                    unfilled_amount: 0,
                    limit_price: 150,
                    execution_price: Some(145),
                    fee_asset_id: Some(zylith_core::AssetId("STRK".into())),
                    fee_amount: 1,
                ...
                zylith_core::OrderExecutionReport {
                    batch_id: BatchId("batch-eth-usdc-42".into()),
                    pair_id: PairId("ETH/USDC".into()),
                    order_commitment: zylith_core::OrderCommitment("0xbeef".into()),
                    order_report_auth_tag: Some("tag-2".into()),
                    funding_note_commitment: zylith_core::NoteCommitment("0xfeed".into()),
                    funding_note_commitments: vec![zylith_core::NoteCommitment("0xfeed".into())],
                    status: "clearing".into(),
                    side: zylith_core::OrderSide::Sell,
                    order_type: zylith_core::OrderType::LimitBatch,
                    time_in_force: zylith_core::TimeInForce::CurrentBatchOnly,
                    execution_preference: zylith_core::ExecutionPreference::PrivateThenExternal,
                    submitted_amount: 1,
        ...
        let mut requested_order_auths = BTreeMap::new();
        requested_order_auths.insert(
            zylith_core::hash::normalize_felt_hex(&order_commitment.0)
                .expect("normalized order commitment"),
            BTreeSet::from(["tag-1".to_string()]),
        );

        let report = super::private_settlement_report_for_multi_pair_published(
            member_batch_id,
            group_id,
            &published,
            Some(&member_binding),
        ...
        .expect("member report");

        assert_eq!(report.batch_id, BatchId(member_batch_id.into()));
        assert_eq!(report.settlement_id, Some(BatchId(group_id.into())));
        assert_eq!(report.pair_id, PairId("STRK/USDC".into()));
        assert_eq!(report.price_base_scale, 1);
        assert_eq!(report.order_execution_reports.len(), 1);
        assert_eq!(
            report.order_execution_reports[0].order_commitment,
            order_commitment
        );
    }
        ...
        let wrong_order_report_auth_response = app
            ...
            .oneshot(
                Request::builder()
                    ...
                    .header("content-type", "application/json")
                    .body(Body::from(
                        br#"{"output_recovery_key_tags":[],"order_report_auths":[{"order_commitment":"0xabc","order_report_auth_tag":"wrong-order-report-auth"}]}"#
                            .to_vec(),
                    ))
                    .expect("request"),
            )
        ...
        let wrong_recovery_tag_response = app
            ...
            .oneshot(
                Request::builder()
                    ...
                    .header("content-type", "application/json")
                    .body(Body::from(
                        br#"{"output_recovery_key_tags":["0x9999"],"order_report_auths":[]}"#
                            .to_vec(),
                    ))
                    .expect("request"),
            )
        ...
        let recovery_scoped_report_response = app
            ...
            .oneshot(
                Request::builder()
                    ...
                    .header("content-type", "application/json")
                    .body(Body::from(
                        br#"{"output_recovery_key_tags":["0x1234"],"order_report_auths":[]}"#
                            .to_vec(),
                    ))
                    .expect("request"),
            )
