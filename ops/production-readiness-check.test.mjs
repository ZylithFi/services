...
test("production readiness rejects incomplete reference price policy", () => {
  const { env } = fixtureEnv();
  ...
  const result = runReadiness(env);
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /reference price policy must include ETH\/USDC/);
...
function fixtureEnv({ proofOverrides = {} } = {}) {
  ...
  const env = {
    ...
    ZYLITH_NATIVE_MULTI_PAIR_SETTLEMENT_STATEMENT_PROGRAM_ADDRESS: "0x10c",
