...
checkFelt("ZYLITH_NATIVE_MULTI_PAIR_SETTLEMENT_STATEMENT_PROGRAM_ADDRESS");
...
function checkReferencePricePolicy() {
  const policyPath = value("ZYLITH_REFERENCE_PRICE_POLICY_PATH") || "ops/config/reference-price-sources.mainnet.json";
  if (!existsSync(policyPath)) {
    failures.push(`reference price policy is required at ${policyPath}`);
    return;
  }
  let policy;
  try {
    ...
    failures.push(`reference price policy at ${policyPath} is not valid JSON: ${error.message}`);
    return;
  }
  if (policy.version !== 1) failures.push("reference price policy version must be 1");
  if (policy.network !== "mainnet") failures.push("reference price policy network must be mainnet");
  if (policy.description && /last[- ]?cleared/i.test(policy.description) && !/intentionally not/i.test(policy.description)) {
    failures.push("reference price policy must not use last-cleared prices as a fallback");
  }
  if (policy.reference_engine?.primary !== "binance-book-midpoint") {
    failures.push("reference price policy reference_engine.primary must be binance-book-midpoint");
  }
  ...
  for (const required of ["last-cleared-price", "fixed-price", "single-exchange-only"]) {
    if (!forbidden.includes(required)) {
      failures.push(`reference price policy must forbid ${required}`);
    }
  }
  ...
  for (const pair of requiredPairs) {
    ...
    if (!pairPolicy) {
      failures.push(`reference price policy must include ${pair}`);
      continue;
    }
    if (!String(pairPolicy.primary ?? "").startsWith("binance-midpoint:")) {
      failures.push(`reference price policy ${pair}.primary must use Binance midpoint`);
    }
    const confirmations = Array.isArray(pairPolicy.confirmations) ? pairPolicy.confirmations : [];
    if (confirmations.length < 2) {
      failures.push(`reference price policy ${pair}.confirmations must include at least two independent non-Binance confirmations`);
    }
    ...
    if (serialized.includes("last-cleared") || serialized.includes("last cleared") || serialized.includes("\"fixed\"")) {
      failures.push(`reference price policy ${pair} must not include last-cleared or fixed-price fallbacks`);
    }
  }
}
