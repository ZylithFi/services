...
async function priorSlotReuseGuard(proverUrl, renewalPackage, slot, state, args) {
  ...
  for (const prior of priorSlots) {
    ...
    if (proofJobFailed(status)) {
      return slotResult(slot, "awaiting_settlement", `Prior child batch ${prior.batch_id} proof failed; refresh this package before reusing funding notes.`);
    }
    if (proofJobConfirmed(status)) {
      if (status.reuse_state === "no_fill") continue;
      if (status.reuse_state === "matched") {
        return slotResult(slot, "awaiting_wallet_refresh", `Prior child batch ${prior.batch_id} settled; refresh this package before reusing funding notes.`);
      }
